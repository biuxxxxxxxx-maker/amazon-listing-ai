    ...
    try {
      ...
      if (projectId !== "demo") {
        await saveGeneratedResult(
          normalizeGenerationResult(data.result),
          data.model || "mock-local",
        );
      } else {
        setDisplayResult(normalizeGenerationResult(data.result));
        setStatusMessage(
          data.source === "deepseek"
            ? "已调用 DeepSeek 生成结果。当前是 demo 项目，所以没有写入数据库。"
            : "当前未配置 DEEPSEEK_API_KEY，已返回 mock 生成结果。",
        );
      }
    } catch (error) {
      setStatusMessage(
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[15.5rem_minmax(0,1fr)]">
          ...
          >
            ...
            <Card className="mt-4 border-orange-200 bg-amberSoft p-4">
          ...
          >
            <div className="space-y-4">
              <BilingualPanel
                title="常见竞品表达"
                english="Most competing listings focus on foldable storage, easy carrying, laundry organization, and multi-room use."
                chinese="多数竞品会强调可折叠收纳、方便搬运、洗衣整理和多房间使用。"
              />
              <BilingualPanel
                title="可借鉴点"
                english="Keep the wording practical and specific. Buyers respond better to clear use cases than broad lifestyle claims."
                chinese="可以借鉴具体场景表达。比起空泛的生活方式文案，买家更容易理解明确用途。"
              />
              <BilingualPanel
                title="可突破点"
                english="Position the product around small-space organization and quick access instead of only saying it saves space."
                chinese="不要只说节省空间，可以进一步强调小空间整理和快速拿取。"
              />
            </div>
          </ResultBlock>
          ...
          >
            <div className="space-y-4">
              {displayBullets.map((bullet, index) => (
                <Card key={bullet.english} className="p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <Badge tone="dark">Bullet {index + 1}</Badge>
                    <CopyButton text={bullet.english} label="复制英文" className="w-full sm:w-auto" />
                  </div>
                  <div className="mt-4">
                    <LanguageLabel>English Bullet</LanguageLabel>
                    <p className="mt-2 text-base font-semibold leading-7 text-ink">{bullet.english}</p>
                  </div>
                  <ChineseReference>{bullet.chinese}</ChineseReference>
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <InfoPill label="对应卖点" value={bullet.sellingPoint} />
                    <InfoPill label="用户痛点" value={bullet.painPoint} />
                  </div>
                </Card>
              ))}
            </div>
          ...
          >
            <div className="space-y-4">
              {displayFaq.map((item, index) => (
                <Card key={item.question.english} className="p-4">
                  ...
                  <div className="mt-4 grid gap-3">
                    <div>
                      <LanguageLabel>English Question</LanguageLabel>
                      <p className="mt-2 font-semibold leading-7 text-ink">
                        {item.question.english}
                      </p>
                    </div>
                    <ChineseReference compact>{item.question.chinese}</ChineseReference>
                    <div className="pt-1">
                      <LanguageLabel>English Answer</LanguageLabel>
                      <p className="mt-2 font-medium leading-7 text-ink">{item.answer.english}</p>
                    </div>
                    <ChineseReference compact>{item.answer.chinese}</ChineseReference>
                  </div>
                </Card>
              ))}
            </div>
          ...
          >
            <div className="grid gap-4 md:grid-cols-2">
              {displayImages.map((image) => (
                <Card key={image.type} className="p-4">
                  <Badge tone="warm">{image.type}</Badge>
                  <p className="mt-3 text-sm font-semibold text-ink">突出卖点：{image.focus}</p>
                  <p className="mt-2 text-sm leading-6 text-neutral-600">
                    中文说明：{image.guidance}
                  </p>
                </Card>
